...
- **收藏/加入歌单自动绑定本地音乐（默认关闭）**：管理台「边听边存」页新增「收藏自动绑定本地」开关。开启后，
  收藏在线歌曲或把在线歌曲加入自建歌单时先立即生效，随后在后台自动整轨下载到本地曲库并绑定下载好的文件；
  下载完成后播放这些曲目直接走本地文件，更换音乐源或原音源下线后依然可以正常播放，解决换源后收藏歌曲无法播放
  的问题。下载复用整轨拉取与落盘转正链路（含标签/歌词随迁与曲库重扫通知），同曲去重、失败 30 分钟冷却，
  不拖慢前端响应。
